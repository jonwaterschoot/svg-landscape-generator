...
void drawScene() {
  ...
  endX = constrain(endX, baseX1 + baseWidth * 0.35, baseX1 + baseWidth * 0.50);
  ...
  for (int i = 1; i < 10; i++) {
    ...
    // Find intersection with the mountain outline (approximate)
    float shadowLineY2 = shadowLineY1; 
    for (float y = shadowLineY1; y >= peakY - 50; y -= 5) { 
      if (!isPointInsideMountain(shadowLineX - 1, y)) { 
        shadowLineY2 = y;
        break;
      }
    }

    // --- Constrain the shadow line length ---
    shadowLineY2 = constrain(shadowLineY2, peakY, shadowLineY1); // Keep it within the mountain
